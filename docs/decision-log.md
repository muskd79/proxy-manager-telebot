...
| `aup-cleanup` | **Wave 25-pre3 partial:** file moved to `src/lib/telegram/_deprecated/aup.ts` with deprecation header. **Still pending:** migration `050_drop_aup_accepted_at.sql` to drop the DB column, then delete the file. | partial | Wave 25-pre4 (DB migration) | unassigned | new migration `supabase/migrations/050_drop_aup_accepted_at.sql`; remove file from `_deprecated/` | DESIGN_REVIEW_LIVE_2026-05-03.md Pass 7.A |
